...
window.addEventListener('DOMContentLoaded', () => {{
  ...
  renderWL();
  {'toast("' + toast.replace('"','\\"') + '");' if toast else ''}
}});
...
function renderResult(d) {{
  ...
  html += `<div class="card card-teal"><div class="ci">
    ...
    <div class="price-row">
      <div class="price-big">${{fp(p)}}</div>
      <div class="chg ${{d.chg>=0?'up':'dn'}}">${{d.chg>=0?'▲':'▼'}} ${{Math.abs(d.chg).toFixed(2)}}%</div>
    </div>
    ...
    <div class="rng-labels">
      ...
      <span class="rmid">52 · W E E K</span>
  ...
  html += `<div class="card card-gold"><div class="ci">
    <div class="ctitle">◆ &nbsp;FUNDAMENTALS</div>
  ...
  html += d.models.map(m => {{
    ...
    return `<div class="mc">
      ...
      <div class="mbody">
        <div class="mrow">
          <span class="mname ${{m.cls}}">◆ ${{m.name}}</span>
          <span class="mval ${{sc}}">${{v}}</span>
        </div>
  ...
  html += `<div class="comp-box">
    <div class="comp-band"></div>
    <div class="comp-inner">
      <div><div class="comp-label">◈ &nbsp;${{cl}}</div><div class="comp-sub">${{cs}}</div></div>
