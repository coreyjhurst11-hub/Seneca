...
/* ── WATCHLIST PAGE ── */
.wl-page{{padding:14px 13px 32px}}
.wl-page-header{{background:linear-gradient(160deg,var(--card2),var(--card));border:1px solid var(--b2);border-radius:16px;padding:16px 18px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between}}
.wl-page-title{{font-family:'Share Tech Mono',monospace;font-size:.72rem;color:var(--g2);letter-spacing:.14em}}
.wl-page-count{{font-family:'Share Tech Mono',monospace;font-size:.58rem;color:var(--dim)}}
.wl-default-label{{font-family:'Share Tech Mono',monospace;font-size:.6rem;color:var(--muted);letter-spacing:.13em;margin-bottom:7px;display:flex;align-items:center;gap:9px}}
.wl-default-label::after{{content:'';flex:1;height:1px;background:linear-gradient(90deg,var(--border),transparent)}}
.wl-default-sub{{font-size:.75rem;color:var(--dim);font-style:italic;line-height:1.7;margin-bottom:14px}}
.wl-index-grid{{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-bottom:16px}}
.wl-tile{{background:linear-gradient(160deg,var(--card2),var(--card));border:1px solid var(--border);border-radius:14px;padding:14px 13px 12px;cursor:pointer;transition:all .18s;position:relative;overflow:hidden}}
.wl-tile:hover{{transform:translateY(-2px);border-color:var(--b2);box-shadow:0 6px 22px rgba(0,0,0,.35)}}
.wl-tile::before{{content:'';position:absolute;top:0;left:0;right:0;height:2px}}
.wl-tile.gold::before{{background:linear-gradient(90deg,var(--gold),var(--g2))}}
.wl-tile.turq::before{{background:linear-gradient(90deg,var(--turq),var(--t2))}}
.wl-tile.muted::before{{background:linear-gradient(90deg,var(--muted),var(--dim))}}
.wl-tile-ticker{{font-family:'Share Tech Mono',monospace;font-size:.78rem;color:var(--g3);letter-spacing:.1em;font-weight:700;margin-bottom:3px}}
.wl-tile-name{{font-size:.65rem;color:var(--sub);font-style:italic;margin-bottom:8px;line-height:1.4}}
.wl-tile-btn{{font-family:'Share Tech Mono',monospace;font-size:.52rem;color:var(--t3);letter-spacing:.08em}}
.wl-tile-rm{{position:absolute;top:8px;right:9px;background:none;border:none;cursor:pointer;color:var(--dim);font-size:.7rem;line-height:1;padding:2px 4px;transition:color .15s}}
.wl-tile-rm:hover{{color:var(--red)}}
.wl-clear-btn{{display:block;width:100%;background:transparent;border:1px solid var(--border);border-radius:10px;color:var(--dim);font-family:'Share Tech Mono',monospace;font-size:.58rem;letter-spacing:.08em;padding:10px;cursor:pointer;transition:all .2s;margin-top:4px}}
    ...
    <div id="pane-w" class="hidden">
      <div class="wl-page">
        ...
        <div class="wl-page-header">
          ...
          <div class="wl-page-sub" id="wl-page-count">Your saved tickers</div>
        </div>

        <!-- Empty state: default indexes -->
        <div id="wl-default" class="hidden">
          <div class="wl-default-label">◈ &nbsp;MARKET BENCHMARKS</div>
          <div class="wl-default-sub">Add stocks via the ◈ Watchlist button after any analysis. Until then, here are the key indexes to watch.</div>
          <div id="wl-default-grid" class="wl-index-grid"></div>
        </div>

        <!-- User's watchlist -->
        <div id="wl-user-list" class="hidden">
          <div class="wl-default-label">◈ &nbsp;YOUR HOLDINGS</div>
          <div id="wl-user-grid" class="wl-index-grid"></div>
...
const DEFAULT_INDEXES = [
  {{ticker:'SPY',  name:'S&P 500 ETF',          color:'gold'}},
  {{ticker:'QQQ',  name:'Nasdaq 100 ETF',        color:'gold'}},
  {{ticker:'DIA',  name:'Dow Jones ETF',          color:'turq'}},
  {{ticker:'IWM',  name:'Russell 2000 ETF',       color:'turq'}},
  {{ticker:'VOO',  name:'Vanguard S&P 500',       color:'gold'}},
  {{ticker:'VTI',  name:'Total Stock Market ETF', color:'muted'}},
  {{ticker:'GLD',  name:'Gold ETF',               color:'gold'}},
  {{ticker:'TLT',  name:'20yr Treasury Bond ETF', color:'muted'}},
];
...
function renderWL() {{
  ...
  // Also refresh tab pane if it's visible
  if(!document.getElementById('pane-w').classList.contains('hidden')) renderWLPage();
}}
function renderWLPage() {{
  const count = watchlist.length;
  document.getElementById('wl-page-count').textContent = count ? count+' ticker'+(count===1?'':'s')+' saved' : 'Your saved tickers';
  ...
  if (count === 0) {{
    ...
    document.getElementById('wl-default-grid').innerHTML = DEFAULT_INDEXES.map(idx =>
      `<div class="wl-tile ${{idx.color}}" onclick="setQ('${{idx.ticker}}');switchTab('a')">
        <div class="wl-tile-ticker">${{idx.ticker}}</div>
        <div class="wl-tile-name">${{idx.name}}</div>
        <div class="wl-tile-btn">◈ Analyze →</div>
      </div>`
    ).join('');
  }} else {{
    defDiv.classList.add('hidden');
    userDiv.classList.remove('hidden');
    document.getElementById('wl-user-grid').innerHTML = watchlist.map((t,i) => {{
      const colors = ['gold','turq','muted','gold','turq','muted'];
      return `<div class="wl-tile ${{colors[i%colors.length]}}" onclick="setQ('${{t}}');switchTab('a')">
        <div class="wl-tile-ticker">${{t}}</div>
        <div class="wl-tile-name">Tap to analyze</div>
        <div class="wl-tile-btn">◈ Analyze →</div>
        <button class="wl-tile-rm" onclick="event.stopPropagation();removeWL('${{t}}')">✕</button>
      </div>`;
    }}).join('');
  }}
}}
