...
def get_ai_verdict(data):
    ...
    try:
        ...
        p=f"""You are SENECA, a stoic value investing oracle. 3 sentences explaining why {data['name']} ({data['ticker']}) appears {data['verdict_cls']}:
Price ${data['price']:.2f} | Fair Value {f"${data['composite']:.2f}" if data['composite'] else 'N/A'} | P/E {data['pe']:.1f} | Verdict: {data['verdict_text']}
...
def render_app(sub=False, email="", toast=""):
    ...
    # Pre-compute toast JS safely (no backslash inside f-string)
...
/* SEARCH — mobile-safe stacked layout */
...
.cmp-r{{display:flex;justify-content:space-between;padding:5px 11px;border-bottom:1px solid var(--border);font-size:.7rem}}
...
.hero-card-inner{{display:flex;align-items:stretch;gap:0;padding:16px 18px 14px}}
...
.hero-right{{display:flex;align-items:center;gap:0;padding-left:16px;flex-shrink:0}}
...
.hero-margin{{font-size:.72rem;font-weight:600;font-family:'Share Tech Mono',monospace}}
...
/* ── LUXURY SECTION ── */
.lux-section{{background:linear-gradient(160deg,var(--card2),var(--card));border:1px solid var(--border);border-radius:16px;margin-bottom:11px;overflow:hidden;padding:13px 14px}}
.lux-section-title{{font-family:'Share Tech Mono',monospace;font-size:.62rem;color:var(--g2);letter-spacing:.14em;margin-bottom:10px;display:flex;align-items:center;gap:8px}}
...
.vd-icon.up{{color:var(--green)}}.vd-icon.dn{{color:var(--red)}}.vd-icon.fair{{color:var(--gold)}}
  ...
  <div class="hdr">
    <div class="hdr-left">
      ...
      <div class="logo-text">
        ...
        <div class="tag">Intrinsic Value Oracle</div>
      </div>
    </div>
    <div class="hdr-right">
  ...
  <main id="main-scroll">
    ...
    <div id="pane-b" class="hidden">
      <div class="card card-gold"><div class="ci">
        ...
        <div style="font-size:.84rem;color:var(--sub);line-height:1.8;font-style:italic">Named for the Stoic philosopher. Six time-tested valuation frameworks reveal what a stock, ETF, or index is truly worth. Create an account to access from any device.</div>
      </div></div>
      ...
      <div class="form-card"><div class="fn">Fed Model (ETFs/Indexes)</div><div class="feq">Price × (Earnings Yield ÷ 10yr Treasury)</div><div class="fdesc">Compares earnings yield to the risk-free rate.</div></div>
...
function syncHeader() {{
  if (userEmail) {{
    ...
    if (userSub) document.getElementById('btn-sub').style.display = 'none';
    else document.getElementById('btn-sub').style.display = '';
  }} else {{
...
// ── THE subscribe flow — simple and direct ────────────────────────────────────
...
async function launchStripe() {{
  ...
  try {{
    ...
    if (d.url) {{
      ...
      // No Stripe configured — demo mode
      userSub = true;
      syncHeader();
      closeModal('modal-pay');
      toast('✦ Demo mode: full access granted!');
    }}
  }} catch(e) {{
    userSub = true; syncHeader(); closeModal('modal-pay');
    toast('✦ Demo mode: full access granted!');
  }} finally {{
...
async function doSignup() {{
  ...
  try {{
    ...
    // Auto-launch Stripe after signup
...
async function doLogin() {{
  ...
  try {{
    ...
    enterApp();
    if (userSub) {{ toast('✦ Welcome back! Subscription active.'); }}
    else {{ toast('◆ Signed in. Subscribe for unlimited access.'); }}
  }} catch(e) {{ err.textContent = 'Network error. Try again.'; }}
...
function addWL(ticker) {{
  ...
  watchlist.push(ticker); saveWL(); renderWL();
...
function renderCmp(d1, d2) {{
  ...
  function col(d, win) {{
    return `<div class="cmp-col ${{win?'win':''}}">
      <div class="cmp-hdr">
        <div class="cmp-tkr">${{d.ticker}}${{win?' ✦':''}}</div>
        <div class="cmp-nm">${{d.name}}</div>
        <div class="cmp-px">${{fp(d.price)}}</div>
      </div>
      ...
      <div class="cmp-vd ${{d.verdict_cls}}">${{d.verdict_text}}</div>
    </div>`;
  }}
...
async function loadAI(ticker) {{
  ...
  try {{
    ...
    el.innerHTML = '<span style="color:var(--dim);font-size:.7rem;font-style:italic">AI unavailable</span>';
  }}
}}
...
async function doAnalyze() {{
  ...
  try {{
    ...
    if (r.status === 402) {{
      setStatus('Free lookup used — subscribe for unlimited access', 'var(--g2)');
      openModal('pay'); return;
    }}
    if (!r.ok) {{
      const e = await r.json(); throw new Error(e.error || 'Server error');
    }}
    const d = await r.json();
    lastTicker = d.ticker;
    renderResult(d);
    ...
    document.getElementById('results').innerHTML =
      `<div class="err"><div class="err-title">⚠ Could not fetch data</div><div class="err-body">${{e.message}}</div></div>`;
...
function renderResult(d) {{
  ...
  // ── HERO CARD: price + composite side by side at top ──
  html += `<div class="hero-card">
    ...
    <div class="hero-range">
      ...
      <div class="rng-pct">${{pct!==null?pct.toFixed(0)+'th percentile of 52-week range':'unavailable'}}</div>
    </div>
  </div>`;
  ...
  // ── MODELS ──
  html += `<div class="lux-section-title" style="margin-bottom:9px">◆ &nbsp;${{isEtf?'INDEX VALUATION MODELS':'VALUATION MODELS'}}</div>`;
  html += d.models.map(m => {{
    const v = m.value&&m.value>0 ? fp(m.value) : 'N/A';
    const sc = m.sig_cls==='down'?'dn':m.sig_cls;
    return `<div class="mc">
      <div class="mbar ${{m.stripe}}"></div>
      <div class="mbody">
        <div class="mrow"><span class="mname ${{m.cls}}">◆ ${{m.name}}</span><span class="mval ${{sc}}">${{v}}</span></div>
        <div class="msig ${{sc}}">${{m.sig_txt}}</div>
        <div class="mfm">${{m.formula}}</div>
      </div>
    </div>`;
  }}).join('');
...
function setStatus(msg, col) {{
  ...
  e.textContent = msg; e.style.color = col || 'var(--dim)';
}}
function fp(v) {{
  if (!v || v<=0) return 'N/A';
...
def api_pdf():
    ...
    try:
        ...
        ai_html = f"<div style='background:#f0f8f5;border:1px solid #1e7a6a;border-radius:8px;padding:14px;margin:12px 0'><div style='font-size:9px;letter-spacing:3px;color:#1e7a6a;text-transform:uppercase;margin-bottom:8px;font-weight:600'>SENECA AI ANALYSIS</div><p style='font-size:11px;line-height:1.7;font-style:italic;margin:0'>{ai}</p></div>" if ai else ""
...
<div class="header"><div><div style="font-size:28px;font-weight:300;letter-spacing:6px;color:#c88a1a">SENECA</div><div style="font-size:10px;color:#888;font-style:italic">Intrinsic Value Oracle</div></div><div style="font-size:10px;color:#888;text-align:right">Generated {datetime.now().strftime('%B %d, %Y')}<br/>Educational purposes only</div></div>
...
<div style="background:#fdf5e0;border:2px solid #c88a1a;border-radius:10px;padding:16px;display:flex;justify-content:space-between;align-items:center;margin:16px 0"><div><div style="font-size:9px;letter-spacing:3px;color:#888;text-transform:uppercase">Seneca Composite</div></div><div style="font-size:28px;font-weight:300;color:#c88a1a">{fp(data['composite'])}</div></div>
<div style="border-left:4px solid {vc};padding:12px 16px;background:#fafafa;border-radius:0 8px 8px 0;margin:12px 0"><div style="font-size:14px;font-weight:600;color:{vc};margin-bottom:4px">{data['verdict_text']}</div><div style="font-size:11px;color:#666;font-style:italic">{data['verdict_detail']}</div></div>
{ai_html}
