#!/usr/bin/env python3
"""
SENECA — Intrinsic Value Oracle v3.1
Fixes: Subscribe button now launches Stripe correctly for all user states.
"""

import os, math, io, hashlib
...
def create_user(email, pw):
    ...
    users[email] = {"pw": hash_pw(pw), "subscribed": False, "created": datetime.now().isoformat()}
...
def update_user(email, **kwargs):
    users = load_users(); email = email.lower().strip()
    if email in users: users[email].update(kwargs); save_users(users)

# ── ETF detection ─────────────────────────────────────────────────────────────
ETF_TICKERS = {
    "SPY","QQQ","IWM","DIA","VTI","VOO","VEA","VWO","GLD","SLV",
    "TLT","IEF","LQD","HYG","XLF","XLK","XLE","XLV","XLI","XLB",
    "ARKK","ARKG","ARKW","IVV","AGG","BND","VIG","SCHD","DGRO",
    "NOBL","SDY","DVY","VYM","HDV","SPYG","SPYV","IWF","IWD",
    "VBK","VBR","VO","VB","VV","EFA","EEM","IEMG","ACWI","VT",
    "MCHI","FXI","EWJ","USO","UNG","SQQQ","TQQQ","SPXU","SPXL","UPRO",
}

def is_etf_or_index(ticker, info):
    if ticker.upper() in ETF_TICKERS: return True
    return (info.get("quoteType") or "").upper() in ("ETF","MUTUALFUND","INDEX","FUTURE")

def index_fair_value(price, div_y_pct, earnings_yield_pct, pe, hist_pe=17.0):
    results = {}
    if earnings_yield_pct > 0: results["fed_model"] = price * (earnings_yield_pct / 4.3)
    if pe > 0: results["pe_reversion"] = price * (hist_pe / pe)
    div = price * (div_y_pct / 100)
    if div > 0: results["ddm"] = div / (0.08 - 0.03)
    return results
...
def resolve_ticker(query):
    ...
    try:
        ...
        if quotes:
            for q in quotes:
                if q.get("quoteType","").upper() == "EQUITY": return q["symbol"]
            return quotes[0]["symbol"]
    except: pass
    return query.upper()

# ── Valuation math ────────────────────────────────────────────────────────────
def graham_number(e, b): return math.sqrt(22.5*e*b) if e>0 and b>0 else None
def graham_growth(e, g): return e*(8.5+2*g)*4.4/4.5 if e>0 and g else None
def buffett_dcf(e, g):
    if e<=0 or not g: return None
    r,d = min(g/100,.25),.09
    return sum(e*(1+r)**y/(1+d)**y for y in range(1,11))+(e*(1+r)**10*15)/(1+d)**10
def lynch_peg(e, g): return e*g if e>0 and g>0 else None
...
def fetch_quote(query):
    ...
    if not price: raise ValueError(f"No data for '{query}'. Try the ticker symbol directly.")
    info = t.info
    def g(key,fb=0.0):
        try: v=info.get(key); f=float(v); return f if math.isfinite(f) else fb
        except: return fb
    ...
    if is_fund:
        ...
        for key,mname,formula,sc,cls in [
            ...
            ("ddm",         "DIVIDEND DISCOUNT", "Annual Dividend ÷ (8% return − 3% growth)",   "muted","muted"),
        ]:
            v=idx_vals.get(key)
            sig_cls,sig_txt=get_signal(v,price) if (v and v>0) else ("na","Insufficient data")
            models_out.append({"name":mname,"formula":formula,"stripe":sc,"cls":cls,"value":v,"sig_cls":sig_cls,"sig_txt":sig_txt})
        valid=[m["value"] for m in models_out if m["value"] and m["value"]>0]
        comp=sum(valid)/len(valid) if valid else None
        asset_type="etf"
    else:
        vals={"gn":graham_number(eps,bvps),"gg":graham_growth(eps,growth),"buf":buffett_dcf(eps,growth),
              "lyn":lynch_peg(eps,growth),"sim":simons_quant(price,pe or 1,pb or 1,roe,mom),"dcf":fcf_dcf(fcf,growth)}
        comp=composite(vals); models_out=[]
        for key,mname,formula,sc,cls in [
    ...
    if comp and comp>0:
        ...
        elif m<=-30: verdict_text,verdict_detail,verdict_cls=f"✦ AVOID · {abs(m):.0f}% above fair value","Significant optimism priced in beyond fundamentals.","down"
    ...
    return {"ticker":ticker,"name":name,"sector":sector,"asset_type":asset_type,
            "price":price,"prev":prev,"eps":eps,"bvps":bvps,"pe":pe,"pb":pb,
            "roe":roe,"growth":growth,"fcf":fcf,"lo52":lo52,"hi52":hi52,
            "mom":mom,"chg":chg,"cap":cap,"div_y":div_y,"beta":beta,
            "composite":comp,"models":models_out,"verdict_text":verdict_text,
            "verdict_detail":verdict_detail,"verdict_cls":verdict_cls,"earnings_yield":earnings_yield}
...
def get_ai_verdict(data):
    ...
    try:
        ...
        prompt = f"""You are SENECA, a stoic value investing oracle. Write a 3-sentence plain-English explanation of why {data['name']} ({data['ticker']}) — a {asset_note} — appears {data['verdict_cls']} based on:
Price: ${data['price']:.2f} | Composite Fair Value: ${f"{data['composite']:.2f}" if data['composite'] else 'N/A'}
P/E: {data['pe']:.1f} | Div Yield: {data['div_y']:.2f}% | Beta: {data['beta']:.2f}
Verdict: {data['verdict_text']}
Be direct, speak like a wise investor. No disclaimers. No bullets. Just 3 flowing sentences."""
...
def build_pdf_html(data, ai_text=None):
    ...
    for m in data["models"]:
        rows+=f"<tr><td>{m['name']}</td><td style='font-family:monospace;font-size:10px'>{m['formula']}</td><td style='text-align:right;font-weight:bold'>{fp(m['value'])}</td><td style='text-align:right'>{m['sig_txt']}</td></tr>"
    ai_sec=f"<div class='ai-box'><div class='ai-title'>◆ SENECA AI ANALYSIS</div><p>{ai_text}</p></div>" if ai_text else ""
...
th{{background:#f5ede0;font-size:9px;padding:6px 8px;text-align:left;color:#888;text-transform:uppercase}}
...
.fund-label{{font-size:9px;color:#888;text-transform:uppercase;letter-spacing:1px}}
...
<table><tr><th>Model</th><th>Formula</th><th style="text-align:right">Fair Value</th><th style="text-align:right">Signal</th></tr>{rows}</table>
...
.btn-upgrade{{background:linear-gradient(135deg,var(--gold),var(--gold2));color:var(--bg);border:none;border-radius:10px;padding:6px 12px;font-family:'Cormorant Garamond',serif;font-size:.78rem;font-weight:600;cursor:pointer;white-space:nowrap;transition:all .2s}}
...
.search-box input{{flex:1;min-width:0;width:0;background:transparent;border:none;outline:none;color:var(--gold3);font-family:'Cormorant Garamond',serif;font-size:1.3rem;font-weight:600;text-align:center;text-transform:uppercase;letter-spacing:.12em;caret-color:var(--turq3)}}
...
.sec::before,.sec::after{{content:'';flex:1;height:1px;background:linear-gradient(90deg,transparent,var(--border))}}
...
.compare-inputs input{{flex:1;min-width:0;background:var(--card);border:1px solid var(--border2);border-radius:10px;padding:8px 10px;color:var(--gold3);font-family:'Cormorant Garamond',serif;font-size:1.05rem;font-weight:600;text-align:center;text-transform:uppercase;letter-spacing:.1em;outline:none}}
...
<div class="hero" id="hero">
  ...
  <div class="hero-cta">
    ...
    <button class="btn-ghost" onclick="handleSubscribeClick()">✦ &nbsp;Subscribe — $9/mo</button>
  </div>
  <div class="hero-models">
    <span class="hm-badge">Graham · Buffett · Lynch</span>
    <span class="hm-badge">Fed Model · ETF Support</span>
    <span class="hm-badge">AI Verdict · PDF Export</span>
...
<div class="hdr">
  ...
  <div class="hdr-right">
    ...
    <button class="btn-upgrade" id="upgrade-btn" onclick="handleSubscribeClick()">✦ $9/mo</button>
  </div>
</div>
</header>
...
<main id="scroll">
  ...
  <div id="pane-b" class="hidden">
    ...
    <div class="pricing-card"><div class="pricing-band"></div><div class="pricing-inner">
      ...
      <button class="btn-subscribe" onclick="handleSubscribeClick()" style="max-width:240px">✦ &nbsp;Subscribe Now</button>
    </div></div>
    ...
    <div class="form-card"><div class="form-name">Fed Model (ETFs)</div><div class="form-eq">Price × (Earnings Yield ÷ 10yr Treasury)</div><div class="form-desc">Compares earnings yield to the risk-free rate.</div></div>
    ...
    <div class="disc"><p>✦ Seneca is for educational and research purposes only. Not financial advice.</p></div>
  </div>
</main>
</div>
...
<div class="modal-overlay" id="modal-paywall">
  <div class="modal"><div class="modal-band"></div><div class="modal-body">
    ...
    <div class="modal-sub">You've used your free lookup.<br/>Subscribe for unlimited access from any device.</div>
    ...
    <button class="btn-subscribe" id="paywall-sub-btn" onclick="paywallSubscribe()">✦ &nbsp;Subscribe Now — $9/mo</button>
...
<div class="modal-overlay" id="modal-auth">
  <div class="modal"><div class="modal-band"></div><div class="modal-body">
    ...
    <div class="modal-sub" id="auth-sub">Create your account, then subscribe to unlock everything.</div>
...
<div class="modal-overlay" id="modal-user">
  <div class="modal"><div class="modal-band"></div><div class="modal-body">
    ...
    <button class="btn-subscribe" id="user-sub-btn" onclick="closeUserMenu();startStripe()" style="display:none">✦ &nbsp;Subscribe Now — $9/mo</button>
...
let authMode = 'signup';
let pendingSubscribe = false; // true when we should auto-launch Stripe after auth
let watchlist = [];
let lastData = null;
...
// ── THE KEY FUNCTION: handles all subscribe button clicks ─────────────────
function handleSubscribeClick() {{
  if (isSubscribed) {{
    // Already subscribed — just enter app
    showToast('✦ You already have full access!');
    enterApp();
    return;
  }}
  if (currentUser) {{
    // Logged in but not subscribed — go straight to Stripe
    startStripe();
  }} else {{
    // Not logged in — show auth first, then auto-Stripe after
...
async function startStripe() {{
  ...
  try {{
    ...
    if (d.url) {{
      ...
      document.getElementById('upgrade-btn').style.display = 'none';
      document.getElementById('user-btn').classList.remove('hidden');
      showToast('✦ Demo mode: full access granted!');
    }}
  }} catch(e) {{
    isSubscribed = true;
    document.getElementById('upgrade-btn').style.display = 'none';
...
function updateAuthUI() {{
  ...
  document.getElementById('auth-sub').textContent = s
    ? 'Create your account, then subscribe to unlock everything.'
    : 'Welcome back to SENECA.';
...
async function doAuth() {{
  ...
  try {{
    ...
    }} else if (pendingSubscribe) {{
      // They clicked Subscribe before auth — launch Stripe now
      showToast('◆ Account created! Launching checkout…');
      setTimeout(startStripe, 800);
    }} else {{
      showToast('◆ Account created! Subscribe for unlimited access.');
    }}
    pendingSubscribe = false;
...
function showUserMenu() {{
  ...
  if (isSubscribed) {{
    ss.textContent = '✦ Active subscription — unlimited access';
    ss.style.color = 'var(--green)'; sb.style.display = 'none';
  }} else {{
    ss.textContent = 'No active subscription';
    ss.style.color = 'var(--dim)'; sb.style.display = 'block';
  }}
...
function renderWatchlist() {{
  ...
  items.innerHTML = watchlist.map(t =>
    `<span class="wl-item" onclick="pick('${{t}}')">${{t}}<button class="wl-remove" onclick="event.stopPropagation();removeFromWatchlist('${{t}}')">✕</button></span>`
  ).join('');
}}
...
async function go() {{
  ...
  try {{
    ...
    if (r.status===402) {{ st('Free lookup used','var(--gold2)'); openPaywall(); return; }}
...
def index():
    ...
    sub=False
    if email:
        u=get_user(email); sub=u.get("subscribed",False) if u else False
...
def api_quote():
    ...
    email=session.get("user_email","")
    is_sub=session.get("subscribed",False)
    if email and not is_sub:
        u=get_user(email)
        if u: is_sub=u.get("subscribed",False)
    lookups=session.get("lookups",0)
    if not is_sub and lookups>=1: return jsonify({"error":"PAYWALL"}),402
    try:
        data=fetch_quote(query); session["lookups"]=lookups+1; return jsonify(data)
...
def api_ai_verdict():
    ...
    try: return jsonify({"verdict":get_ai_verdict(fetch_quote(ticker))})
    except Exception as e: return jsonify({"error":str(e)}),500
...
def api_export_pdf():
    ...
    try:
        data=fetch_quote(ticker); ai_text=get_ai_verdict(data)
        html_content=build_pdf_html(data,ai_text)
        try:
            from weasyprint import HTML
            buf=io.BytesIO(HTML(string=html_content).write_pdf()); buf.seek(0)
...
def create_checkout():
    ...
    try:
        email=session.get("user_email","")
        kwargs=dict(payment_method_types=["card"],mode="subscription",
