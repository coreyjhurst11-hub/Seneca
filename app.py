...
.hdr-clock{{font-family:'Share Tech Mono',monospace;font-size:.52rem;color:var(--dim);letter-spacing:.08em}}
/* Right side actions */
.hdr-right{{display:flex;align-items:center;gap:8px;flex-shrink:0;padding:11px 0}}
.btn-login{{background:transparent;color:var(--sub);border:1px solid rgba(110,69,20,.7);border-radius:9px;padding:7px 13px;font-family:'Share Tech Mono',monospace;font-size:.54rem;cursor:pointer;white-space:nowrap;letter-spacing:.07em;transition:all .2s}}
.btn-login:hover{{border-color:var(--b2);color:var(--g2);background:rgba(110,69,20,.15)}}
.btn-sub{{background:linear-gradient(135deg,var(--gold) 0%,var(--g2) 60%,var(--g3) 100%);color:var(--bg);border:none;border-radius:9px;padding:7px 14px;font-family:'Cormorant Garamond',serif;font-size:.84rem;font-weight:600;cursor:pointer;white-space:nowrap;letter-spacing:.04em;box-shadow:0 2px 16px rgba(200,138,26,.3),inset 0 1px 0 rgba(255,255,255,.1);transition:all .22s}}
.btn-sub:hover{{transform:translateY(-1px);box-shadow:0 6px 24px rgba(200,138,26,.45)}}
.btn-acct{{background:rgba(110,69,20,.2);color:var(--g2);border:1px solid var(--b2);border-radius:9px;padding:7px 13px;font-family:'Share Tech Mono',monospace;font-size:.54rem;cursor:pointer;white-space:nowrap;letter-spacing:.06em;transition:all .2s}}
.btn-acct:hover{{background:rgba(110,69,20,.35);color:var(--g3)}}
...
<div id="shell" class="shell">
  <div class="hdr">
    ...
    <!-- Center clock -->
    <div class="hdr-center"><div class="hdr-clock" id="hdr-clock"></div></div>
    <!-- Right actions -->
    <div class="hdr-right">
...
const DEFAULT_INDEXES = [
  {{ticker:'SPY',  name:'SPDR S&P 500 ETF',       color:'gold'}},
  {{ticker:'QQQ',  name:'Invesco Nasdaq 100 ETF',  color:'gold'}},
  {{ticker:'DIA',  name:'SPDR Dow Jones ETF',       color:'turq'}},
  {{ticker:'IWM',  name:'iShares Russell 2000 ETF', color:'turq'}},
  {{ticker:'VOO',  name:'Vanguard S&P 500 ETF',    color:'gold'}},
  {{ticker:'VTI',  name:'Vanguard Total Market',   color:'muted'}},
  {{ticker:'GLD',  name:'SPDR Gold Shares',         color:'gold'}},
  {{ticker:'TLT',  name:'iShares 20yr Treasury',   color:'muted'}},
];

// Price cache: ticker -> {{price, chg, name, ts}}
const priceCache = {{}};
...
function buildCard(ticker, name, color, removable) {{
  ...
  const priceHtml = cached
    ? `<div class="wl-card-price">${{cached.price}}</div>
       <div class="wl-card-chg ${{cached.cls}}">${{cached.chg}}</div>`
    : `<div class="wl-card-price-loading"><span class="wl-shimmer" style="width:52px;height:14px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
       <div class="wl-card-chg neutral" style="margin-top:4px"><span class="wl-shimmer" style="width:36px;height:9px">&nbsp;&nbsp;&nbsp;&nbsp;</span></div>`;
  const displayName = (cached && cached.name && cached.name !== ticker) ? cached.name : name;
  ...
  return `<div class="wl-card" id="wlcard-${{ticker}}">
    ...
    <div class="wl-card-body">
      <div class="wl-card-left">
        ...
        <div class="wl-card-name">${{displayName}}</div>
      </div>
...
function renderWLPage() {{
  ...
  sub.textContent = count ? count+' ticker'+(count===1?'':'s')+' · tap to analyze' : 'Benchmarks · tap to analyze';
  ...
  if (count === 0) {{
    ...
    const colors = ['gold','gold','turq','turq','gold','muted','gold','muted'];
    document.getElementById('wl-default-cards').innerHTML =
      DEFAULT_INDEXES.map((idx,i) => buildCard(idx.ticker, idx.name, colors[i], false)).join('');
    fetchWLPrices(DEFAULT_INDEXES.map(x=>x.ticker));
  }} else {{
    defDiv.classList.add('hidden');
    userDiv.classList.remove('hidden');
    const colors = ['gold','turq','muted','gold','turq','muted','gold','turq'];
    document.getElementById('wl-user-cards').innerHTML =
      watchlist.map((t,i) => buildCard(t, '', colors[i%colors.length], true)).join('');
    fetchWLPrices([...watchlist]);
  }}
}}
...
async function fetchWLPrices(tickers) {{
  ...
  const results = await Promise.allSettled(
    tickers.map(t => fetch('/api/quote?q='+encodeURIComponent(t)).then(r=>r.ok?r.json():null))
  );
  results.forEach((res, i) => {{
    const t = tickers[i];
    if(res.status==='fulfilled' && res.value && res.value.price) {{
      const d = res.value;
      const price = '$'+d.price.toLocaleString('en-US',{{minimumFractionDigits:2,maximumFractionDigits:2}});
      const chgSign = d.chg >= 0 ? '▲' : '▼';
      const cls = d.chg >= 0 ? 'up' : 'dn';
      priceCache[t] = {{
        price, name: d.name,
        chg: chgSign+' '+Math.abs(d.chg).toFixed(2)+'%',
        cls
      }};
    }}
    // Update DOM in place
    ...
    if(el && priceCache[t]) {{
      ...
      el.innerHTML = `<div class="wl-card-price">${{c.price}}</div>
        <div class="wl-card-chg ${{c.cls}}">${{c.chg}}</div>`;
      // Update name if we got it
      const nameEl = el.closest('.wl-card')?.querySelector('.wl-card-name');
      if(nameEl && c.name && c.name !== t) nameEl.textContent = c.name;
    }}
  }});
...
async function refreshWLPrices() {{
  ...
  // Clear cache for these so cards show shimmer
  tickers.forEach(t => delete priceCache[t]);
  renderWLPage();
  // prices will be fetched inside renderWLPage
}}
