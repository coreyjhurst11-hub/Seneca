...
@app.route("/api/price")
def api_price():
    """Lightweight endpoint: price + change only, uses fast_info (no t.info call)."""
    tickers = request.args.get("tickers","").strip()
    if not tickers: return jsonify({"error":"No tickers"}), 400
    import yfinance as yf
    results = {}
    for raw in tickers.split(",")[:10]:
        t = raw.strip().upper()
        if not t: continue
        try:
            ...
            if price:
                ...
                results[t] = {"price": price, "chg": round(chg, 2)}
        except Exception:
            pass
...
/* Quotes section */
.wl-quotes-section{{padding:0 12px 32px}}
.wl-quote-card{{display:flex;gap:13px;align-items:flex-start;background:linear-gradient(160deg,rgba(30,19,8,.75),rgba(20,13,5,.85));border:1px solid rgba(78,48,16,.35);border-radius:14px;padding:16px 16px 14px;margin-bottom:8px;transition:border-color .2s}}
.wl-quote-card:hover{{border-color:rgba(110,69,20,.55)}}
.wl-quote-mark{{font-size:2rem;color:rgba(110,69,20,.4);font-family:Georgia,serif;line-height:1;flex-shrink:0;margin-top:-3px}}
.wl-quote-body{{flex:1;min-width:0}}
.wl-quote-text{{font-size:.86rem;color:var(--sub);font-style:italic;line-height:1.75;margin-bottom:8px}}
.wl-quote-author{{font-family:'Share Tech Mono',monospace;font-size:.52rem;color:var(--g2);letter-spacing:.13em}}
...
const WL_QUOTES = [
  ...
  {{text:"The individual investor should act consistently as an investor, not as a speculator.", author:"Benjamin Graham"}},
...
function buildQuotesSection() {{
  ...
  const shuffled = [...WL_QUOTES].sort(() => Math.random() - 0.5);
  return `<div class="wl-quotes-section">
    <div class="wl-section-label" style="padding:24px 16px 10px">◈ &nbsp;WORDS OF WISDOM</div>
    ${{shuffled.map(q => `
      <div class="wl-quote-card">
        <div class="wl-quote-mark">❝</div>
        <div class="wl-quote-body">
          <div class="wl-quote-text">${{q.text}}</div>
          <div class="wl-quote-author">— ${{q.author.toUpperCase()}}</div>
        </div>
      </div>`).join('')}}
  </div>`;
}}
...
function renderWLPage() {{
  ...
  if (count === 0) {{
    ...
    qEl.innerHTML = buildQuotesSection();
  }} else {{
